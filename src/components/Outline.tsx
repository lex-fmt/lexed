...
function getExpandedPaths(
  ...
  activeSymbol: DocumentSymbol | null,
  basePath: string = ''
): Set<string> {
...
export function Outline({ currentFile, editor, cursorLine }: OutlineProps) {
  ...
  const renderSymbols = (items: DocumentSymbol[], depth = 0, basePath = '') => {
    return items.map((item, index) => {
      ...
      const isExpanded =
        !collapsed.has(path) && (autoExpandedPaths.has(path) || !hasChildren || depth === 0)
      ...
      return (
        <div key={path}>
          ...
          {hasChildren && isExpanded && <div>{renderSymbols(item.children!, depth + 1, path)}</div>}
        </div>
      )
    })
  }
