...
export function SettingsDialog({ isOpen, onClose }: SettingsDialogProps) {
  const { settings, updateEditorSettings, updateFormatterSettings, updateSpellcheckSettings } =
    useSettings()
    ...
    >
      <div className="w-[720px] bg-panel border border-border rounded-lg shadow-xl flex flex-col max-h-[80vh]">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-6">
            {activeTab === 'ui' && (
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLocalEditorSettings((prev) => ({ ...prev, showRuler: e.target.checked }))
                      }
                  ...
                  <div className="flex items-center justify-between">
                    <label
                      htmlFor="ruler-width"
                      className={cn('text-sm', !localEditorSettings.showRuler && 'opacity-50')}
                    >
                      Ruler width
                    </label>
                    <input
                ...
                <div className="flex items-center justify-between">
                  <label htmlFor="vim-mode" className="text-sm">
                    Enable Vim Mode
                  </label>
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) =>
                      setLocalEditorSettings((prev) => ({ ...prev, vimMode: e.target.checked }))
                    }
                    className="accent-primary"
                  />
                </div>
              ...
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label
                        htmlFor="session-before"
                        className="text-xs text-muted-foreground uppercase tracking-wide"
                      >
                        Session blank lines before
                      </label>
                      <input
                        id="session-before"
                        type="number"
                    ...
                    <div>
                      <label
                        htmlFor="session-after"
                        className="text-xs text-muted-foreground uppercase tracking-wide"
                      >
                        Session blank lines after
                      </label>
                      <input
                        id="session-after"
                        type="number"
            ...
            {activeTab === 'spellcheck' && (
              <div className="space-y-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <label htmlFor="spellcheck-enabled" className="text-sm">
                      Enable Spellcheck
                    </label>
                    <input
                      type="checkbox"
                  ...
                  <div className="flex items-center justify-between">
                    <label
                      ...
                      className={cn('text-sm', !localSpellcheckSettings.enabled && 'opacity-50')}
                    >
                      Language
                    </label>
                    <select
        ...
        <div className="flex items-center justify-end gap-2 px-4 py-3 border-t border-border bg-panel-hover/30">
          <button onClick={onClose} className="px-3 py-1.5 text-sm hover:bg-panel-hover rounded">
            Cancel
          </button>
          <button
            onClick={() => {
