...
export function FileTree({
  ...
  const shouldIncludeEntry = useCallback(
    (entryPath: string, isDirectory: boolean) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener(
        FILE_TREE_REFRESH_EVENT as any,
        handleCustomRefresh as EventListener
      )
    }
  }, [refreshTree])
  ...
  const renderTree = (entries: FileEntry[], depth = 0) => {
    return entries.map((entry) => {
      ...
      const isAncestorOfSelection =
        entry.isDirectory && normalizedSelectedFile
      ...
      return (
        <div key={entry.path}>
          ...
          {entry.isOpen && entry.children && <div>{renderTree(entry.children, depth + 1)}</div>}
        </div>
      )
    })
  }
  ...
  return (
    <div
      className="h-full bg-panel overflow-y-auto text-foreground relative"
