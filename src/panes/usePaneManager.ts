...
export function usePaneManager({
  ...
  const updateRowsAfterPaneRemoval = useCallback(
    (paneId: string, remainingPaneIds: string[]) => {
  ...
  const handleTabSelect = useCallback(
    (paneId: string, tabId: string) => {
      setPanes((prev) =>
        prev.map((pane) => (pane.id === paneId ? { ...pane, activeTabId: tabId } : pane))
      )
      setActivePaneId(paneId)
    },
  ...
  const handleTabDrop = useCallback(
    (targetPaneId: string, data: TabDropData) => {
      ...
      setPanes((prev) => {
        ...
        if (existingTab) {
          return prev.map((pane) =>
            pane.id === targetPaneId ? { ...pane, activeTabId: existingTab.id } : pane
          )
        }
  ...
  const handlePaneFileLoaded = useCallback(
    (paneId: string, path: string | null) => {
      setPanes((prev) =>
        prev.map((pane) => (pane.id === paneId ? { ...pane, currentFile: path } : pane))
      )
    },
    [setPanes]
  )

  const handlePaneCursorChange = useCallback(
    (paneId: string, line: number) => {
      setPanes((prev) =>
        prev.map((pane) => (pane.id === paneId ? { ...pane, cursorLine: line } : pane))
      )
    },
    [setPanes]
  )
