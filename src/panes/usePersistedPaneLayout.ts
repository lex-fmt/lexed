...
export function usePersistedPaneLayout(
  createTabFromPath: (path: string) => Tab
): PersistedPaneLayoutResult {
  ...
  const [paneRows, setPaneRows] = useState<PaneRowState[]>(() =>
    defaultLayoutRef.current!.rows.map(withRowDefaults)
  )
  const [activePaneId, setActivePaneId] = useState<string>(
    () => defaultLayoutRef.current!.activePaneId
  )
      ...
      try {
        ...
        if (layout && Array.isArray(layout.panes) && layout.panes.length > 0) {
          const hydrated = layout.panes.map<PaneState>((pane) => ({
            ...
            activeTabId:
              pane.activeTab && pane.tabs.includes(pane.activeTab)
                ? pane.activeTab
                : pane.tabs[0] || null,
            currentFile: null,
            cursorLine: 0,
          ...
          const unreferenced = hydrated.map((p) => p.id).filter((id) => !referencedIds.has(id))
          ...
          const savedActiveId =
            layout.activePaneId && hydrated.some((p) => p.id === layout.activePaneId)
              ? layout.activePaneId
