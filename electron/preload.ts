...
contextBridge.exposeInMainWorld('ipcRenderer', {
  on(...args: Parameters<typeof ipcRenderer.on>) {
    ...
    const subscription = (event: IpcRendererEvent, ...listenerArgs: unknown[]) =>
  ...
  fileNew: (defaultPath?: string) =>
    ipcRenderer.invoke('file-new', defaultPath) as Promise<{
      filePath: string
      content: string
    } | null>,
  ...
  fileChecksum: (filePath: string) =>
    ipcRenderer.invoke('file-checksum', filePath) as Promise<string | null>,
  ...
  loadTestFixture: (fixtureName: string) =>
    ipcRenderer.invoke('test-load-fixture', fixtureName) as Promise<{
      path: string
      content: string
    }>,
  ...
  getOpenTabs: () =>
    ipcRenderer.invoke('get-open-tabs') as Promise<{
  ...
  updateMenuState: (state: { hasOpenFile: boolean; isLexFile: boolean }) =>
  ...
  shareWhatsApp: (content: string) =>
    ipcRenderer.invoke('share-whatsapp', content) as Promise<void>,
  ...
  setEditorSettings: (settings: { showRuler: boolean; rulerWidth: number; vimMode: boolean }) =>
  ...
  onShowToast: (callback: (type: 'success' | 'error' | 'info', message: string) => void) => {
    const handler = (
