...
// Isolate user-data dir when requested (set by e2e tests so they don't
// pick up the developer's real persisted settings — in particular the
// spellcheck language, which determines which Hunspell dict gets parsed
...
function getCustomDictionaryPath(): string {
  // Always under userData. The app bundle is read-only in production,
  // and in dev this keeps the repo from collecting every word a
  // developer or test adds via "Add to dictionary" — the old behavior
  // wrote into dictionaries/custom.dic, which got committed, leaked
  // personal words, and polluted e2e tests across runs.
...
ipcMain.handle('spellcheck-load-supplement', async () => {
  // Shared tech/programming/OS/networking vocabulary layered on top of
  // the SCOWL-derived Hunspell base dictionaries (which miss words like
  // `lifecycle`, `amd64`, `stdin`). The supplement is language-agnostic:
  // tech terms are loanwords in most languages, so every spellcheck
  // language benefits. Generated by scripts/generate-dictionary-supplement.py
  // from streetsidesoftware/cspell-dicts (MIT) + a hand-curated gap list.
  const supplementPath = path.join(getDictionariesPath(), 'supplement.dic')
  try {
    const content = await fs.readFile(supplementPath, 'utf-8')
    const lines = content.split('\n').map((w) => w.trim())
    // First line is the Hunspell count header; skip it if numeric
    const start = /^\d+$/.test(lines[0] ?? '') ? 1 : 0
    return lines.slice(start).filter((w) => w.length > 0)
  } catch {
    return []
  }
})
