...
test.describe('Settings', () => {
  test('loads latest formatter values when switching tabs', async () => {
    ...
    try {
      ...
      await expect(page.locator('input#session-before')).toHaveValue(
        String(customFormatter.sessionBlankLinesBefore)
      )
      await expect(page.locator('input#session-after')).toHaveValue(
        String(customFormatter.sessionBlankLinesAfter)
      )
      await expect(page.locator('input#max-blank-lines')).toHaveValue(
        String(customFormatter.maxBlankLines)
      )
      await expect(page.locator('input#indent-spaces')).toHaveValue(
        String(customFormatter.indentString.length)
      )
      await expect(page.locator('input#unordered-marker')).toHaveValue(
        customFormatter.unorderedSeqMarker
      )
      await expect(
        page.locator('label:has-text("Normalize list markers") input[type="checkbox"]')
      ).not.toBeChecked()
      await expect(
        page.locator('label:has-text("Normalize verbatim markers") input[type="checkbox"]')
      ).not.toBeChecked()
      await expect(
        page.locator('label:has-text("Preserve trailing blank lines") input[type="checkbox"]')
      ).toBeChecked()
      await expect(
        page.locator('label:has-text("Format automatically on save") input[type="checkbox"]')
      ).toBeChecked()
    } finally {
  ...
  test('persists UI and formatter settings after closing dialog', async () => {
    ...
    try {
      ...
      await page
        .locator('label:has-text("Normalize list markers") input[type="checkbox"]')
        .uncheck()
      await page
        .locator('label:has-text("Normalize verbatim markers") input[type="checkbox"]')
        .uncheck()
      await page
        .locator('label:has-text("Preserve trailing blank lines") input[type="checkbox"]')
        .check()
      await page
